...
class Engine(object):
    ...
    def create_users_table(self):
        ...
        '''
        keys_on = 'PRAGMA foreign_keys = ON'
        stmnt = 'CREATE TABLE users(user_id INTEGER PRIMARY KEY AUTOINCREMENT,nickname TEXT UNIQUE, regDate INTEGER, lastLogin INTEGER, timesviewed INTEGER,UNIQUE(user_id, nickname))'
    ...
    def create_users_profile_table(self):
        ...
        '''
        keys_on = 'PRAGMA foreign_keys = ON'
        stmnt = 'CREATE TABLE users_profile(\
...
class Connection(object):
    ...
    def delete_message(self, messageid):
        ...
        # Enable the foreign keys support
        self.set_foreign_keys_support()
        # Cursor and row initialization
        self.con.row_factory = sqlite3.Row
        cur = self.con.cursor()
        # The DELETE query for deleting a message with the given messageid
        query = "DELETE FROM messages WHERE message_id=" + str(messageid)
        # Execute the query and commit the connection
        cur.execute(query)
        self.con.commit()

        # If row count is less than one, no row has been deleted so we return
        # False
    ...
    def modify_message(self, messageid, title, body, editor="Anonymous"):
        ...
        # The query for updating the message information
        # query = "UPDATE messages " +\
                # "SET title=" + title + "," +\
                # "body=" + body + "," +\
                # "editor_nickname=" + editor + " " +\
                # "WHERE message_id=" + str(messageid)
        query = "UPDATE messages SET title=?,body=?,editor_nickname=? WHERE\
                message_id=?"
        ...
        self.con.row_factory = sqlite3.Row
        cur = self.con.cursor()
        # Execute our query and commit
        cur.execute(query,pvalue)
        self.con.commit()

        # If row count is less than one, no message has been updated so we
        # return False
        return (1 <= cur.rowcount)
    ...
    def create_message(self, title, body, sender="Anonymous",
        ...
        message_exists_query = "SELECT * FROM messages WHERE message_id=?"
        pvalue = (replyto,)
        cur.execute(message_exists_query, pvalue)
        # cur.rowcount gives us the number of affected rows. Since SELECT
        # query doesn't affect anything, we have to check the size of the list
        # when we try to fetch all the rows.
        if (1 > len(cur.fetchall())):
            replyto = None

        # Get the user_id using username(sender)
        user_id_query = "SELECT user_id FROM users WHERE nickname=?"
        pvalue = (sender,)
        cur.execute(user_id_query, pvalue)
        # Process the response
        row = cur.fetchone()
        user_id = row["user_id"]
        # We expected one row, but just in case clean the cursor before
        # executing something else
        cur.fetchall()

        query = "INSERT INTO messages (title, body, timestamp, ip, \
                                        timesviewed, reply_to, user_nickname, \
                                        user_id)\
                VALUES (?,?,?,?,?,?,?,?)"
        pvalues = (title, body, time.mktime(datetime.now().timetuple()),
                ipaddress, 0, replyto, sender, user_id)
                # "VALUES (" + title + "," + body + "," +\
                # time.mktime(datetime.now().timetuple()) + "," +\
                # ipaddress + "," + "0" + "," + reply_to + "," + sender + "," +\
                # user_id + ")"

        # Execute the query
        cur.execute(query, pvalues)
        self.con.commit()
        cur.fetchall()
        # Check the rowcount if the INSERT query was successful
        if (1 > cur.rowcount):
            raise ForumDatabaseError("The database could not be modified")
            return None
 
        # Get the id of the message and return it
        query = "SELECT message_id FROM messages WHERE rowid=?"
        pvalue = (cur.lastrowid,)
        cur.execute(query, pvalue)
        row = cur.fetchone()
    ...
    def get_user_id(self, nickname):
        ...
        pvalue = (nickname,)
        self.set_foreign_keys_support()
        self.con.row_factory = squlite3.Row
        cur = self.con.cursor()
        cur.execute(query, pvalue)
        row = cur.fetchone()
        if row is None:
